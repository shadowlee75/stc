...
const STCPresets = (() => {
  ...
  // ---------- TC 저장 설비 (도면 "TC details") ----------
  // 도면 판독: 전장 45,000 (System length 44,875) · 폭 14,700 · 전고 19,600
  //   베이 6 × 피치 5,650 → CL 5,550 / 11,200 / 16,850 / 22,500 / 28,150 / 33,800 (bay1→bay6 = 28,250)
  //   단 6 × 피치 2,900 → 1,500 / 4,400 / 7,300 / 10,200 / 13,100 / 16,000 (+상부 3,550 = 19,550 ≈ 19,600)
  //   통로 2개(크레인 2대), 각 모듈 = 랙 2,050 + 통로 3,200 + 랙 2,050 → (7,300)×2 + 100 = 14,700
  //   우측 타워 = bay6 CL 33,800 + 6,000 + 5,200 = 45,000
  //   입고 Entry from TC coolers 70 JPH (상부) · 출고 Exit to insp. & polish 70 JPH (하부)
  // 크레인 제원은 도면에 없어 [[carBodyRev0]] 값을 준용한다 — 확인 필요.
  function tcStorageDwg() {
    return {
      schema: 'stcsim/scenario@1',
      name: 'TC 저장 설비 — 도면 6 Bay × 6 Level × 2통로',
      note: '출처: TC details 도면. 통로당 72셀(양측), 2통로 합계 144셀. 입출고 70 JPH 균형. 크레인 속도·이재시간·가동률은 도면에 없어 차체 Rev.0 값 준용 — 스펙서로 확인 요.',
      seedBase: 12345,
      run: { durationSec: 8 * 3600, warmupSec: 3600, tsBinSec: 300, sampleSec: 10 },
      rack: {
        bays: 6, levels: 6, sides: 2,
        x0: 2.725, length: 33.9, height: 16.0, aisleLength: 45.0,
        bayX: [5.55, 11.20, 16.85, 22.50, 28.15, 33.80],
        levelY: [1.5, 4.4, 7.3, 10.2, 13.1, 16.0],
        depth: 1, initialFill: 0.5, initialFillHours: 2,
        cellDepth: 2.05, aisleWidth: 3.2, loadSize: { l: 4.8, w: 1.9, h: 1.5 },
      },
      crane: {
        axes: {
          x: { v: 175 / 60, a: 0.4, d: 0.4, creepDist: 0, creepV: 0, tDelay: 0 },
      ...
      stations: [
        { id: 'E_TC', name: '입고 TC coolers (상부)', kind: 'in', x: 39.8, y: 14.75, capacity: 2,
          arrival: { type: 'takt', ratePerHour: 70, jitterPct: 10 } },
        { id: 'A_INSP', name: '출고 검사·폴리시 (하부)', kind: 'out', x: 39.8, y: 1.5, capacity: 2,
          arrival: { type: 'takt', ratePerHour: 70, jitterPct: 10, minStock: 1 }, retrieval: 'any', takeAway: null },
      ],
      cranes: [
        { id: 'STC1', zone: { x: [0, 45.0], y: [0, 16.0] }, home: 'E_TC', x0: 39.8, y0: 14.75 },
      ],
      policy: { mode: 'DC', pairing: 'nearest', priority: 'oldest-first', storageRule: 'closest-open', retrievalCell: 'nearest', agingLimit_s: null },
      parallelAisles: 2,
      targets: { rhoMax: 0.8, maxQueue: 3, doneRatioMin: 0.98 },
      hist: { cycle: { binSec: 5, maxSec: 300 }, wait: { binSec: 15, maxSec: 1800 } },
    };
  ...
  const list = [
    ...
    { key: 'tcStorageDwg', label: 'TC 저장 설비 — 도면 6×6 × 2통로', make: tcStorageDwg },
  ...
  return { version: '0.2.0', list, get, carBodyRev0, tcStorageDwg, generic20x10, largeRackValidation, bozerWhite, clone };
})();
